...
namespace ActiveUp.Net.Common.Rfc2047
{
	internal class EncodedWord : TokenBase
	{
		...
		private bool IsBase64
		{
			get
			{
				return _encoding.Equals("B", StringComparison.OrdinalIgnoreCase);
			}
		}
		...
		public bool CanBeConcatenatedWith(TokenBase other)
		{
			...
			EncodedWord encodedWord = other as EncodedWord;
			if (encodedWord == null || encodedWord._charset != _charset || encodedWord._encoding != _encoding)
			{
				return false;
			}
		...
		public static EncodedWord Concat(EncodedWord first, EncodedWord second)
		{
			...
			return new EncodedWord(first._rawValue.TrimEnd('=') + second._rawValue, first._charset, first._encoding)
			{
				NextSeparator = second.NextSeparator,
				NextWord = second.NextWord
			};
		}
	}
}
