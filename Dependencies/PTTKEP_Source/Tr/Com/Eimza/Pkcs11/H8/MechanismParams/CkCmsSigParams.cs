...
namespace Tr.Com.Eimza.Pkcs11.H8.MechanismParams
{
	internal class CkCmsSigParams : IMechanismParams, IDisposable
	{
		...
		{
			_lowLevelStruct.CertificateHandle = 0uL;
			...
			_lowLevelStruct.RequestedAttributesLen = 0uL;
			_lowLevelStruct.RequiredAttributes = IntPtr.Zero;
			_lowLevelStruct.RequiredAttributesLen = 0uL;
			...
			{
				byte[] array = ConvertUtils.ULongToBytes(signingMechanism.Value);
			...
			{
				byte[] array2 = ConvertUtils.ULongToBytes(digestMechanism.Value);
