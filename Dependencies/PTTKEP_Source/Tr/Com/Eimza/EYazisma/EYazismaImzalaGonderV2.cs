...
{
	internal class EYazismaImzalaGonderV2
	{
				...
				if (smartCardPasswd != null)
				{
					...
					lock (Sync)
					{
						...
						case EYazismaPaketTur.ETEbligat:
							...
							{
								EyLog.Log("Imzala Gönder", EyLogTuru.HATA, "ETebligat Paket Türünde MailTypeID Parametresi Boş Bırakılamaz.");
								eyImzalaGonderSonuc.Durum = 161;
								eyImzalaGonderSonuc.HataAciklama = "ETebligat Paket Türünde MailTypeID Parametresi Boş Bırakılamaz.";
								return eyImzalaGonderSonuc;
							}
							break;
						...
						SmimeCreaterV2 smimeCreaterV = new SmimeCreaterV2();
						byte[] array;
						try
						{
							array = smimeCreaterV.CreateWithSmartCard(kepHesap.kepHesap, to, cc, bcc, subject, content, attachments, mailType2, mailTypeId, icerikTur, sReader);
						}
						catch (Exception ex3)
						{
							LOG.Error(ex3);
			...
			try
			{
				...
				if (smartCardPasswd != null)
				{
					...
					lock (Sync)
					{
						...
						try
						{
							array = smimeCreaterV.CreateWithSmartCard(kepHesap.kepHesap, to, cc, bcc, subject, content, attachments, mailType2, mailTypeId, icerikTur, sReader, ozetAlg);
						}
						catch (Exception ex3)
						{
							LOG.Error(ex3);
