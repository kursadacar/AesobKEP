...
namespace Tr.Com.Eimza.Org.BouncyCastle.Tsp
{
    internal class TspUtil
	{
		...
		public static string HttpRequestContentType;

		public static string[] HttpResponseContentType;
		...
		static TspUtil()
		{
			...
			HttpRequestContentType = "application/timestamp-query";
			HttpResponseContentType = new string[2] { "application/timestamp-reply", "application/timestamp-response" };
		...
		public static ICollection GetSignatureTimestamps(SignerInformation signerInfo)
		{
			...
			Tr.Com.Eimza.Org.BouncyCastle.Asn1.Cms.AttributeTable unsignedAttributes = signerInfo.UnsignedAttributes;
			if (unsignedAttributes != null)
			{
				foreach (Tr.Com.Eimza.Org.BouncyCastle.Asn1.Cms.Attribute item in unsignedAttributes.GetAll(PkcsObjectIdentifiers.IdAASignatureTimeStampToken))
				{
					foreach (Asn1Encodable attrValue in item.AttrValues)
					{
						try
						{
							TimeStampToken timeStampToken = new TimeStampToken(Tr.Com.Eimza.Org.BouncyCastle.Asn1.Cms.ContentInfo.GetInstance(attrValue.ToAsn1Object()));
		...
		{
			return ((string)digestNames[digestAlgOID]) ?? digestAlgOID;
		}
