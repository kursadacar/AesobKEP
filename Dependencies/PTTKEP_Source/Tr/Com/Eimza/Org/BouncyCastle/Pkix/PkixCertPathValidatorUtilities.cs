...
using Tr.Com.Eimza.Org.BouncyCastle.Asn1.IsisMtt;
...
{
	internal class PkixCertPathValidatorUtilities
	{
		...
		internal static ISet GetCompleteCrls(DistributionPoint dp, object cert, DateTime currentDate, PkixParameters paramsPKIX)
		{
			...
			if (set2.IsEmpty)
			{
				if (cert is IX509AttributeCertificate)
				{
					...
					X509Name x509Name = iX509AttributeCertificate.Issuer.GetPrincipals()[0];
					throw new Exception("No CRLs found for issuer \"" + ((x509Name != null) ? x509Name.ToString() : null) + "\"");
				}
				X509Certificate x509Certificate = (X509Certificate)cert;
				X509Name issuerDN = x509Certificate.IssuerDN;
				throw new Exception("No CRLs found for issuer \"" + ((issuerDN != null) ? issuerDN.ToString() : null) + "\"");
			}
			return set2;
		}
