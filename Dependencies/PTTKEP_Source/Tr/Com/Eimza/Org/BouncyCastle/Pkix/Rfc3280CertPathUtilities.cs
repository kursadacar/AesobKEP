...
{
	internal class Rfc3280CertPathUtilities
	{
						...
						for (int i = 0; i < list.Count; i++)
						{
							...
							while (enumerator2.MoveNext())
							{
								...
								if (!flag)
								{
									...
									set4.Add(text);
									PkixPolicyNode pkixPolicyNode3 = new PkixPolicyNode(Platform.CreateArrayList(), num, set4, pkixPolicyNode, qualifierSet, text, false);
		...
		internal static ISet[] ProcessCrlA1ii(DateTime currentDate, PkixParameters paramsPKIX, X509Certificate cert, X509Crl crl)
		{
			...
			{
				throw new Exception("Cannot extract issuer from CRL." + ((ex != null) ? ex.ToString() : null), ex);
			}
		...
		internal static void ProcessCertA(PkixCertPath certPath, PkixParameters paramsPKIX, int index, AsymmetricKeyParameter workingPublicKey, X509Name workingIssuerName, X509Certificate sign)
		{
			...
			{
				string[] obj = new string[5] { "IssuerName(", null, null, null, null };
				obj[1] = ((issuerPrincipal != null) ? issuerPrincipal.ToString() : null);
				obj[2] = ") does not match SubjectName(";
				obj[3] = ((workingIssuerName != null) ? workingIssuerName.ToString() : null);
				obj[4] = ") of signing certificate.";
				throw new PkixCertPathValidatorException(string.Concat(obj), null, certPath, index);
			}
		}
