...
{
	internal class PkixNameConstraintValidator
	{
		...
		private void CheckPermittedDNS(ISet permitted, string dns)
		{
			...
			while (enumerator.MoveNext())
			{
				...
				if (WithinDomain(dns, text) || dns.ToUpper().Equals(text.ToUpper()))
				{
					return;
				}
			}
		...
		private void checkExcludedDNS(ISet excluded, string dns)
		{
			...
			{
				string text = (string)enumerator.Current;
				if (WithinDomain(dns, text) || Platform.CompareIgnoreCase(dns, text) == 0)
				{
		...
		private void unionEmail(string email1, string email2, ISet union)
		{
			...
			{
				if (Platform.CompareIgnoreCase(email2.Substring(email1.IndexOf('@') + 1), email1) == 0)
				{
					union.Add(email1);
					return;
				}
		...
		private void intersectEmail(string email1, string email2, ISet intersect)
		{
			...
			{
				if (Platform.CompareIgnoreCase(email2.Substring(email2.IndexOf('@') + 1), email1) == 0)
				{
					intersect.Add(email2);
				}
			}
