...
namespace Tr.Com.Eimza.Org.BouncyCastle.Math.EC
{
	...
	{
		private static readonly int[] INTERLEAVE2_TABLE = new int[256]
		{
		...
		{
			return (x & long.MinValue) | Interleave3_21to63((int)x & 0x1FFFFF) | (Interleave3_21to63((int)((ulong)x >> 21) & 0x1FFFFF) << 1) | (Interleave3_21to63((int)((ulong)x >> 42) & 0x1FFFFF) << 2);
		}
		...
		private static long Interleave3_21to63(int x)
		{
			...
			return ((INTERLEAVE3_TABLE[(uint)x >> 14] & 0xFFFFFFFFu) << 42) | ((num2 & 0xFFFFFFFFu) << 21) | (num & 0xFFFFFFFFu);
		}
		...
		private static long Interleave3_13to65(int x)
		{
			...
			return ((INTERLEAVE5_TABLE[(uint)x >> 7] & 0xFFFFFFFFu) << 35) | (num & 0xFFFFFFFFu);
		}
		...
		{
			return (x & long.MinValue) | INTERLEAVE7_TABLE[(int)x & 0x1FF] | (INTERLEAVE7_TABLE[(int)((ulong)x >> 9) & 0x1FF] << 1) | (INTERLEAVE7_TABLE[(int)((ulong)x >> 18) & 0x1FF] << 2) | (INTERLEAVE7_TABLE[(int)((ulong)x >> 27) & 0x1FF] << 3) | (INTERLEAVE7_TABLE[(int)((ulong)x >> 36) & 0x1FF] << 4) | (INTERLEAVE7_TABLE[(int)((ulong)x >> 45) & 0x1FF] << 5) | (INTERLEAVE7_TABLE[(int)((ulong)x >> 54) & 0x1FF] << 6);
		}
		...
		private static long Interleave4_16to64(int x)
		{
			...
			return ((INTERLEAVE4_TABLE[(uint)x >> 8] & 0xFFFFFFFFu) << 32) | (num & 0xFFFFFFFFu);
		}

		private static long Interleave2_32to64(int x)
		{
			int num = INTERLEAVE2_TABLE[x & 0xFF] | (INTERLEAVE2_TABLE[((uint)x >> 8) & 0xFF] << 16);
			return (((INTERLEAVE2_TABLE[((uint)x >> 16) & 0xFF] | (INTERLEAVE2_TABLE[(uint)x >> 24] << 16)) & 0xFFFFFFFFu) << 32) | (num & 0xFFFFFFFFu);
		}
