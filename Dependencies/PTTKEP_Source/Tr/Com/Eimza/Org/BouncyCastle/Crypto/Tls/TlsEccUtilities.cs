...
{
	internal abstract class TlsEccUtilities
	{
		...
		public static byte[] ReadSupportedPointFormatsExtension(byte[] extensionData)
		{
			...
			MemoryStream memoryStream = new MemoryStream(extensionData, false);
			byte num = TlsUtilities.ReadUint8(memoryStream);
			if (num < 1)
			{
				throw new TlsFatalAlert(50);
			}
			byte[] array = TlsUtilities.ReadUint8Array(num, memoryStream);
			TlsProtocol.AssertEmpty(memoryStream);
		...
		{
			if (a.Curve.Equals(b.Curve) && a.G.Equals(b.G) && a.N.Equals(b.N))
			{
				return a.H.Equals(b.H);
			}
			return false;
		}
