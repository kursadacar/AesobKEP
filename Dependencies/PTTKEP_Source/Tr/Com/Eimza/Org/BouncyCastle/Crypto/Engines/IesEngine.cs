...
namespace Tr.Com.Eimza.Org.BouncyCastle.Crypto.Engines
{
	internal class IesEngine
	{
		...
		public byte[] ProcessBlock(byte[] input, int inOff, int inLen)
		{
			...
			byte[] z = BigIntegers.AsUnsignedByteArray(agree.GetFieldSize(), n);
			if (!forEncryption)
			{
				return DecryptBlock(input, inOff, inLen, z);
			}
			return EncryptBlock(input, inOff, inLen, z);
		}
	}
}
