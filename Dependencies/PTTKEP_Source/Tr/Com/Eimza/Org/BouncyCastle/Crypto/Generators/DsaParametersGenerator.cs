...
{
	internal class DsaParametersGenerator
	{
		...
		protected virtual DsaParameters GenerateParameters_FIPS186_3()
		{
			...
			while (true)
			{
				...
				BigInteger bigInteger2 = BigInteger.One.ShiftLeft(N - 1).Add(bigInteger).Add(BigInteger.One)
					.Subtract(bigInteger.Mod(BigInteger.Two));
				...
				for (int i = 0; i < num3; i++)
				{
					...
					while (num4 <= num2)
					{
						...
						if (num4 == num2)
						{
							bigInteger4 = bigInteger4.Mod(BigInteger.One.ShiftLeft(n));
						}
