...
namespace Tr.Com.Eimza.Org.BouncyCastle.Crypto.Tls
{
	internal class TlsClientProtocol : TlsProtocol
	{
			...
			case 14:
			{
				...
				if ((uint)(num - 2) <= 5u)
				{
					...
					if (tlsCredentials != null && tlsCredentials is TlsSignerCredentials)
					{
						...
						{
							signatureAndHashAlgorithm = tlsSignerCredentials.SignatureAndHashAlgorithm;
		...
		protected virtual void ReceiveServerHelloMessage(MemoryStream buf)
		{
			...
			if (!Arrays.Contains(mOfferedCipherSuites, num) || num == 0 || num == 255 || !TlsUtilities.IsValidCipherSuiteForVersion(num, protocolVersion))
			{
				throw new TlsFatalAlert(47);
			}
			...
			if (TlsExtensionsUtilities.HasExtendedMasterSecretExtension(mServerExtensions) != mSecurityParameters.extendedMasterSecret)
			{
				throw new TlsFatalAlert(40);
			}
			if (mServerExtensions != null)
			{
				foreach (int key in mServerExtensions.Keys)
				{
					if (key != 65281)
					{
						...
						if (key != 23)
						{
							bool mResumedSession2 = mResumedSession;
						}
					}
				}
			}
