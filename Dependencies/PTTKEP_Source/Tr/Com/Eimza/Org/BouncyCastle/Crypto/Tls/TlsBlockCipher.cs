...
{
	internal class TlsBlockCipher : TlsCipher
	{
		...
		public TlsBlockCipher(TlsContext context, IBlockCipher clientWriteCipher, IBlockCipher serverWriteCipher, IDigest clientWriteDigest, IDigest serverWriteDigest, int cipherKeySize)
		{
			...
			encryptCipher.Init(true, parameters3);
			decryptCipher.Init(false, parameters4);
		}
		...
		public virtual byte[] EncodePlaintext(long seqNo, byte type, byte[] plaintext, int offset, int len)
		{
			...
			if (useExplicitIV)
			{
				...
				encryptCipher.Init(true, new ParametersWithIV(null, array2));
		...
		public virtual byte[] DecodeCiphertext(long seqNo, byte type, byte[] ciphertext, int offset, int len)
		{
			...
			if (!encryptThenMac)
			{
				...
				if (!Arrays.ConstantTimeAreEqual(mReadMac.CalculateMacConstantTime(seqNo, type, ciphertext, offset, num6, num2 - size, randomData), b2) || num4 == 0)
				{
					throw new TlsFatalAlert(20);
				}
			}
