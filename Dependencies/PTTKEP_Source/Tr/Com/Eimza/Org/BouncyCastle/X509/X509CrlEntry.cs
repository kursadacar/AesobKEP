...
{
	internal class X509CrlEntry : X509ExtensionBase
	{
		private readonly CrlEntry c;

		private readonly bool isIndirect;

		private readonly X509Name previousCertificateIssuer;

		private readonly X509Name certificateIssuer;

		public BigInteger SerialNumber
		{
			get
			{
				return c.UserCertificate.Value;
			}
		}
		...
		public DateTime RevocationDate
		{
			get
			{
				return c.RevocationDate.ToDateTime();
			}
		}

		public virtual BigInteger ReasonCode
		{
			get
			{
				try
				{
					X509Extension extension = GetExtension(X509Extensions.ReasonCode);
					if (extension != null && extension.Value != null)
					{
						return new CrlReason(DerEnumerated.GetInstance(Asn1Object.FromByteArray(extension.Value.GetOctets()))).Value;
					}
					return null;
				}
				catch (Exception)
