...
{
	internal class CmsEnvelopedDataStreamGenerator : CmsEnvelopedGenerator
	{
		...
		private DerInteger Version
		{
			get
			{
				return new DerInteger((_originatorInfo != null || _unprotectedAttributes != null) ? 2 : 0);
			}
		}
		...
		private Stream Open(Stream outStream, AlgorithmIdentifier encAlgID, ICipherParameters cipherParameters, Asn1EncodableVector recipientInfos)
		{
			try
			{
				...
				Stream stream = CmsUtilities.CreateBerOctetOutputStream(berSequenceGenerator3.GetRawOutputStream(), 0, false, _bufferSize);
