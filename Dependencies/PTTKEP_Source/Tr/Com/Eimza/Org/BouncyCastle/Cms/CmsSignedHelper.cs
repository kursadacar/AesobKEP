...
using Tr.Com.Eimza.Org.BouncyCastle.Asn1.Eac;
...
namespace Tr.Com.Eimza.Org.BouncyCastle.Cms
{
	...
	{
		public static readonly CmsSignedHelper Instance;
		...
		private static readonly IDictionary onlydigestAlgs;
		...
		static CmsSignedHelper()
		{
			...
			onlydigestAlgs = Platform.CreateHashtable();
			...
			onlydigestAlgs.Add(PkcsObjectIdentifiers.MD2.Id, "MD2");
			onlydigestAlgs.Add(PkcsObjectIdentifiers.MD4.Id, "MD4");
			onlydigestAlgs.Add(PkcsObjectIdentifiers.MD5.Id, "MD5");
			onlydigestAlgs.Add(OiwObjectIdentifiers.IdSha1.Id, "SHA1");
			onlydigestAlgs.Add(NistObjectIdentifiers.IdSha224.Id, "SHA224");
			onlydigestAlgs.Add(NistObjectIdentifiers.IdSha256.Id, "SHA256");
			onlydigestAlgs.Add(NistObjectIdentifiers.IdSha384.Id, "SHA384");
			onlydigestAlgs.Add(NistObjectIdentifiers.IdSha512.Id, "SHA512");
			onlydigestAlgs.Add(TeleTrusTObjectIdentifiers.RipeMD128.Id, "RIPEMD128");
			onlydigestAlgs.Add(TeleTrusTObjectIdentifiers.RipeMD160.Id, "RIPEMD160");
			onlydigestAlgs.Add(TeleTrusTObjectIdentifiers.RipeMD256.Id, "RIPEMD256");
			onlydigestAlgs.Add(CryptoProObjectIdentifiers.GostR3411.Id, "GOST3411");
			onlydigestAlgs.Add("1.3.6.1.4.1.5849.1.2.1", "GOST3411");
		...
		public string GetDigestAlgFromSignatureAlg(string signatureOid)
		{
			string obj = (string)digestAlgs[signatureOid];
			foreach (DictionaryEntry onlydigestAlg in onlydigestAlgs)
			{
				if (onlydigestAlg.Value.Equals(obj))
				{
					return (string)onlydigestAlg.Key;
				}
			}
			return null;
		}
		...
		public string GetDigestAlgWithEncyptionAlg(string signatureOid, string separator = "")
		{
			string digestAlgName = GetDigestAlgName(signatureOid);
			string encryptionAlgName = GetEncryptionAlgName(signatureOid);
			return digestAlgName + separator + "with" + separator + encryptionAlgName;
		}

		public string GetEncyptionAlgWithDigestAlg(string signatureOid, string separator = "")
		{
			string digestAlgName = GetDigestAlgName(signatureOid);
			string encryptionAlgName = GetEncryptionAlgName(signatureOid);
			return encryptionAlgName + separator + "with" + separator + digestAlgName;
		}
		...
		public IDigest GetDigestInstance(string algorithm)
		{
			try
			{
		...
		public ISigner GetSignatureInstance(string algorithm)
		{
			return SignerUtilities.GetSigner(algorithm);
		}

		public IX509Store CreateAttributeStore(string type, Asn1Set certSet)
		{
		...
		public IX509Store CreateCertificateStore(string type, Asn1Set certSet)
		{
			IList list = Platform.CreateArrayList();
			if (certSet != null)
			{
		...
		public IX509Store CreateCrlStore(string type, Asn1Set crlSet)
		{
			IList list = Platform.CreateArrayList();
			if (crlSet != null)
			{
		...
		private void AddCertsFromSet(IList certs, Asn1Set certSet)
		{
			foreach (Asn1Encodable item in certSet)
			{
				try
				{
					...
					{
						certs.Add(new X509Certificate(asn1Object.GetEncoded()));
					}
				}
				catch (Exception e)
				{
		...
		private void AddCrlsFromSet(IList crls, Asn1Set crlSet)
		{
			...
			foreach (Asn1Encodable item in crlSet)
			{
				try
				{
					crls.Add(new X509Crl(item.GetEncoded()));
				}
				catch (Exception e)
				{
