...
{
	internal class CmsAuthenticatedDataStreamGenerator : CmsAuthenticatedGenerator
	{
		...
		protected Stream Open(Stream outStr, AlgorithmIdentifier macAlgId, ICipherParameters cipherParameters, Asn1EncodableVector recipientInfos)
		{
			try
			{
				...
				Stream output = CmsUtilities.CreateBerOctetOutputStream(berSequenceGenerator3.GetRawOutputStream(), 0, false, _bufferSize);
				IMac mac = MacUtilities.GetMac(macAlgId.ObjectID);
				mac.Init(cipherParameters);
				return new CmsAuthenticatedDataOutputStream(new TeeOutputStream(output, new MacOutputStream(mac)), mac, berSequenceGenerator, berSequenceGenerator2, berSequenceGenerator3);
			}
