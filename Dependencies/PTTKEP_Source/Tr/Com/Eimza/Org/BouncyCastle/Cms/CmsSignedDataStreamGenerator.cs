...
{
	internal class CmsSignedDataStreamGenerator : CmsSignedGenerator
	{
			...
			public SignerInfo Generate(DerObjectIdentifier contentType, AlgorithmIdentifier digestAlgorithm, byte[] calculatedDigest)
			{
				try
				{
					...
					if (_sAttr != null)
					{
						...
						Tr.Com.Eimza.Org.BouncyCastle.Asn1.Cms.AttributeTable attributeTable = _sAttr.GetAttributes(baseParameters);
					...
					if (_unsAttr != null)
					{
						...
						Tr.Com.Eimza.Org.BouncyCastle.Asn1.Cms.AttributeTable attributes = _unsAttr.GetAttributes(baseParameters2);
					...
					AlgorithmIdentifier encAlgorithmIdentifier = CmsSignedGenerator.GetEncAlgorithmIdentifier(new DerObjectIdentifier(_encOID), defaultX509Parameters);
		...
		protected override void AddSignerCallback(SignerInformation si)
		{
		...
		public Stream Open(Stream outStream, string signedContentType, bool encapsulate, Stream dataOutputStream = null)
		{
			if (outStream == null)
			{
			...
			Stream s = (encapsulate ? CmsUtilities.CreateBerOctetOutputStream(berSequenceGenerator3.GetRawOutputStream(), 0, true, _bufferSize) : null);
		...
		private static Stream AttachDigestsToOutputStream(ICollection digests, Stream s)
		{
			...
			{
				stream = GetSafeTeeOutputStream(stream, new DigOutputStream(digest));
			}
			return stream;
		}
