...
{
	internal class CmsSignedGenerator
	{
		protected static readonly string Data;
		...
		internal readonly IList _certs = Platform.CreateArrayList();

		internal readonly IList _crls = Platform.CreateArrayList();

		internal readonly IList _signers = Platform.CreateArrayList();

		internal readonly IDictionary _digests = Platform.CreateHashtable();
		...
		protected internal virtual Asn1Set GetAttributeSet(Tr.Com.Eimza.Org.BouncyCastle.Asn1.Cms.AttributeTable attr)
		{
			if (attr != null)
			{
		...
		public void AddAttributeCertificates(IX509Store store)
		{
			try
			{
				...
				{
					_certs.Add(new DerTaggedObject(false, 2, AttributeCertificate.GetInstance(Asn1Object.FromByteArray(match.GetEncoded()))));
				}
			}
			catch (Exception e)
			{
		...
		protected virtual void AddSignerCallback(SignerInformation si)
		{
		}
