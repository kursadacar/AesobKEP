...
{
	internal class TbsRequest : Asn1Encodable
	{
		...
		private TbsRequest(Asn1Sequence seq)
		{
			...
			{
				requestorName = GeneralName.GetInstance((Asn1TaggedObject)seq[num++], true);
			}
			requestList = (Asn1Sequence)seq[num++];
			if (seq.Count == num + 1)
			{
				requestExtensions = X509Extensions.GetInstance((Asn1TaggedObject)seq[num], true);
			}
		}
