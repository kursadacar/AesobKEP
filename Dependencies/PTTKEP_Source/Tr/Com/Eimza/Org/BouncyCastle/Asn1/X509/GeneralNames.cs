...
{
	internal class GeneralNames : Asn1Encodable
	{
		...
		public static bool HasMatch(GeneralNames iList, X509Name iName)
		{
			for (int i = 0; i < iList.GetNames().Length; i++)
			{
				GeneralName generalName = iList.GetNames()[0];
				if (generalName.TagNo == 5 && generalName.GetDirectoryName().Equivalent(iName))
				{
					return true;
				}
			}
