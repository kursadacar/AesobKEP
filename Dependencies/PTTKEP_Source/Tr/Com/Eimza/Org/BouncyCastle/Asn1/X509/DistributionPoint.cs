...
{
	internal class DistributionPoint : Asn1Encodable
	{
		...
		public X509Name CrlIssuerName
		{
			get
			{
				if (cRLIssuer != null && cRLIssuer.GetNames().Length != 0)
				{
					return X509Name.GetInstance(cRLIssuer.GetNames()[0].Name);
				}
				return null;
			}
		}
		...
		private DistributionPoint(Asn1Sequence seq)
		{
			for (int i = 0; i != seq.Count; i++)
			{
				...
				{
				case 0:
					distributionPoint = DistributionPointName.GetInstance(instance, true);
					break;
				case 1:
					reasons = new ReasonFlags(DerBitString.GetInstance(instance, false));
					break;
				case 2:
					cRLIssuer = GeneralNames.GetInstance(instance, false);
					break;
				}
			}
		}
