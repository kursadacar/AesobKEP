...
namespace Tr.Com.Eimza.Org.BouncyCastle.Asn1.Tsp
{
	internal class TimeStampReq : Asn1Encodable
	{
		...
		public TimeStampReq(DerObjectIdentifier hashAlgorithm, byte[] hash, DerObjectIdentifier tsaPolicy)
		{
			version = new DerInteger(1);
			messageImprint = new MessageImprint(new AlgorithmIdentifier(hashAlgorithm), hash);
			this.tsaPolicy = tsaPolicy;
			nonce = null;
