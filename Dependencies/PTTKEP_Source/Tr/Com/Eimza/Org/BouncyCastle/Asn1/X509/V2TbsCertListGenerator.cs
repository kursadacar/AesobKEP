...
{
	internal class V2TbsCertListGenerator
	{
		...
		public void AddCrlEntry(DerInteger userCertificate, Time revocationDate, int reason, DerGeneralizedTime invalidityDate)
		{
			...
			if (reason != 0)
			{
				...
				{
					throw new ArgumentException("error encoding reason: " + ((ex != null) ? ex.ToString() : null));
				}
			}
			if (invalidityDate != null)
			{
				...
				{
					throw new ArgumentException("error encoding invalidityDate: " + ((ex2 != null) ? ex2.ToString() : null));
				}
			}
			if (list.Count != 0)
			{
