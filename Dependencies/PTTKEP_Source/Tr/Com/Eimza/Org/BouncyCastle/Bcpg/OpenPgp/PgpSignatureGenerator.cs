...
{
	internal class PgpSignatureGenerator
	{
		...
		public PgpSignature Generate()
		{
			...
			{
				array = insertSubpacket(array, new SignatureCreationTime(false, DateTime.UtcNow));
			}
			if (!packetPresent(hashed, SignatureSubpacketTag.IssuerKeyId) && !packetPresent(unhashed, SignatureSubpacketTag.IssuerKeyId))
			{
				array2 = insertSubpacket(array2, new IssuerKeyId(false, privKey.KeyId));
			}
			int num = 4;
			byte[] array4;
			try
