...
namespace Tr.Com.Eimza.Org.BouncyCastle.OpenSsl
{
    internal class MiscPemGenerator : PemObjectGenerator
	{
		...
		private static PemObject CreatePemObject(object obj)
		{
			...
			{
				if (!(obj is Tr.Com.Eimza.Org.BouncyCastle.Asn1.Cms.ContentInfo))
				{
					throw new PemGenerationException("Object type not supported: " + obj.GetType().FullName);
				}
				type = "PKCS7";
				content = ((Tr.Com.Eimza.Org.BouncyCastle.Asn1.Cms.ContentInfo)obj).GetEncoded();
			}
		...
		private static PemObject CreatePemObject(object obj, string algorithm, char[] password, SecureRandom random)
		{
			...
			byte[] array2 = new byte[text2.StartsWith("AES-") ? 16 : 8];
