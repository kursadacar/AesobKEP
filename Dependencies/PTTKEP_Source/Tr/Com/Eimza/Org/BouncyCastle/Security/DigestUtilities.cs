...
namespace Tr.Com.Eimza.Org.BouncyCastle.Security
{
	internal static class DigestUtilities
	{
		...
		public static byte[] CalculateDigestSHA256(byte[] input)
		{
			IDigest digest = GetDigest(DigestAlgorithm.SHA_256.ToString());
			digest.BlockUpdate(input, 0, input.Length);
			return DoFinal(digest);
		}

		public static byte[] CalculateDigest(DerObjectIdentifier algorithmId, byte[] input)
		{
			IDigest digest = GetDigest(algorithmId);
			digest.BlockUpdate(input, 0, input.Length);
			return DoFinal(digest);
		}
