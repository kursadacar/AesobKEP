...
using Tr.Com.Eimza.Org.BouncyCastle.Asn1.BC;
...
{
	internal sealed class PbeUtilities
	{
		...
		public static ICipherParameters GenerateCipherParameters(string algorithm, char[] password, bool wrongPkcs12Zero, Asn1Encodable pbeParameters)
		{
			...
			if (IsPkcs5Scheme2(text))
			{
				...
				byte[] array2 = ((!objectID.Equals(PkcsObjectIdentifiers.RC2Cbc)) ? Asn1OctetString.GetInstance(obj).GetOctets() : RC2CbcParameter.GetInstance(obj).GetIV());
				...
				parameters = MakePbeGenerator((string)algorithmType[text], null, array, salt, iterationCount).GenerateDerivedParameters(objectID.Id, keySize);
		...
		public static object CreateEngine(AlgorithmIdentifier algID)
		{
			...
			{
				return CipherUtilities.GetCipher(PbeS2Parameters.GetInstance(algID.Parameters.ToAsn1Object()).EncryptionScheme.ObjectID);
			}
